...
class RISAM2 : public ISAM2 {
 ...
 public:
  /// @brief Variables
  int mode = 0;
  std::unordered_map<unsigned long, int> num_visited; // <factor_idx, current step>
  std::unordered_map<unsigned long, int> init_num_visited; //  <<factor_idx, initial step>
  std::vector<Point> control_points_lower = {{0, 0}, {0.02, 0.3}, {0.1, 0.6}, {0.2, 0.8}, {0.4, 0.9}, {0.7, 0.98}, {1.0, 1.0}}; // inlier control points
  std::vector<Point> control_points_middle = {{0, 0}, {0.01, 0.4}, {0.05, 0.7}, {0.1, 0.9}, {0.3, 0.95}, {0.6, 0.99}, {1.0, 1.0}}; // mode 1 control points
  std::vector<Point> control_points_upper = {{0, 0}, {0, 0.5}, {0, 0.8}, {0.0, 1.0}, {0.2, 1.0}, {0.5, 1.0}, {1.0, 1.0}}; // outlier control points
  std::vector<Point> control_points;
  std::vector<double> knots = {0, 0, 0, 0, 0.25, 0.5, 0.75, 1, 1, 1, 1};
  int degree = 3;
  
  // maximum step size
  double lower_step_size = 4.0;
  double upper_step_size = 4.0;
  double other_step_size = 4.0;

  double bezier_curve(double t, double p0, double p1, double p2, double p3);
  double N(int i, int k, double t, const std::vector<double>& knots);
  std::vector<Point> bspline_curve(const std::vector<double>& t_values, int degree, const std::vector<double>& knots, const std::vector<Point>& control_points);
   ...
   */
  GaussianFactor::shared_ptr relinearizeFactor_m0(const size_t factor_idx, const KeySet& updateInvolvedKeys,
                                                  const KeySet& relinKeys);
  GaussianFactor::shared_ptr relinearizeFactor_m1(const size_t factor_idx, const KeySet& updateInvolvedKeys,
                                                  const KeySet& relinKeys);
  GaussianFactor::shared_ptr relinearizeFactor_m2(const size_t factor_idx, const KeySet& updateInvolvedKeys,
                                                  const KeySet& relinKeys);
  ...
  void pushBackFactors_m0(const NonlinearFactorGraph& newFactors, bool known_inliers,
  ...
  void pushBackFactors_m1(const NonlinearFactorGraph& newFactors, bool known_inliers,
                          NonlinearFactorGraph* nonlinearFactors, GaussianFactorGraph* linearFactors,
                          VariableIndex* variableIndex, FactorIndices* newFactorsIndices,
                          KeySet* keysWithRemovedFactors, ISAM2UpdateParams updateParams);

  void pushBackFactors_m2(const NonlinearFactorGraph& newFactors, bool known_inliers,
                          NonlinearFactorGraph* nonlinearFactors, GaussianFactorGraph* linearFactors,
                          VariableIndex* variableIndex, FactorIndices* newFactorsIndices,
                          KeySet* keysWithRemovedFactors, ISAM2UpdateParams updateParams);
  ...
  void updateMuInitOnConvergence_m0();

  void updateMuInitOnConvergence_m1();

  void updateMuInitOnConvergence_m2();
 ...
 private:
  std::function<void()> updateMuInitOnConvergence = nullptr;
  std::function<GaussianFactor::shared_ptr(const size_t, const KeySet&, const KeySet&)> relinearizeFactor = nullptr;
  std::function<void(const NonlinearFactorGraph&, bool, NonlinearFactorGraph*, GaussianFactorGraph*, VariableIndex*,
                     FactorIndices*, KeySet*, const ISAM2UpdateParams)>
      pushBackFactors = nullptr;
};  // class RISAM2
}  // namespace risam